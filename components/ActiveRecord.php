...
class ActiveRecord extends BaseActiveRecord
{
     ...
     * {@inheritdoc}
     *
     * @return \simialbi\yii2\rest\ActiveQuery
     * @throws InvalidConfigException
     */
    ...
    {
        $path = Inflector::camel2id(StringHelper::basename(static::class), '-');
    ...
    protected function insertInternal($attributes): bool
    {
        ...
        $values = ['entity' => $this->getDirtyAttributes($attributes)];
        if (false === ($data = static::getDb()->createCommand()->insert(static::modelName(), $values))) {
            return false;
        }
        if (!$href = ($data['link']['href'] ?? false)) {
